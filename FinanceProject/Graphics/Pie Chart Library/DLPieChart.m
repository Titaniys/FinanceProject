...
-(void)drawLegends:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    legendsView.frame = CGRectMake(layerHostingView.pieCenter.x+layerHostingView.pieRadius+OFFSET*2, 10, 170, 200); //150
    ...
    [legendsScrollView setBackgroundColor:[UIColor whiteColor]];
    ...
    legendsView.frame = CGRectMake(legendX, legendY, legendWidth + OFFSET, legendHeight);
    legendsScrollView.frame = CGRectMake(0, 0, legendsView.frame.size.width + OFFSET, legendsView.frame.size.height);
    ...
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(40, y, legendWidth + OFFSET, 25)]; //OFFSET*2
        lbl.text = [NSString stringWithFormat:@"%@",[dataArray objectAtIndex:i]];
        [lbl setFont:[UIFont fontWithName:@"Arial" size:11]];
...
- (NSString *)pieChart:(DLPieChart *)pieChart textForSliceAtIndex:(NSUInteger)index{
    NSString *str =[NSString stringWithFormat:@"%@$",[self.DLDataArray objectAtIndex:index]];
